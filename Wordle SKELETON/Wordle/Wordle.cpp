...
{
    Score s(m, mWinningMove);//create score object
    mCurrentScore = s;//set currentScore
    mBoard.endRound(m, mCurrentScore);//update the board
    mRound++;//increment the Round value
    return( s );
}
...
GAMEOUTCOME  Wordle::determineGameOutcome( ) const
{
    ...
    if(mRound<=TOTALROUNDSALLOWED && mCurrentScore.isExactMatch())
        outcome = GAMEWON;
